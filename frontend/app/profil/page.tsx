...
export default function ProfilPage() {
  ...
  const ucitajAdminPodatke = async () => {
    ...
    try {
      const [resS, resSl] = await Promise.all([
        fetch("https://overflowing-spirit-production-fde5.up.railway.app/api/admin/studenti", { headers }),
        fetch("https://overflowing-spirit-production-fde5.up.railway.app/api/admin/sluzbenici", { headers }),
      ]);
  ...
  const ucitajZaduzenja = async () => {
    try {
      const res = await fetch("https://overflowing-spirit-production-fde5.up.railway.app/api/zaduzenja/aktivna", {
  ...
  const handleRazduzi = async (id: number) => {
    ...
    const res = await fetch(`https://overflowing-spirit-production-fde5.up.railway.app/api/razduzi/${id}`, {
      method: "PUT",
      headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
    });
  ...
  const pretraziWiki = async () => {
    ...
    try {
      const res = await fetch(
        `https://overflowing-spirit-production-fde5.up.railway.app/api/eksterni/istrazi/${wikiPojam}`,
      );
      if (res.ok) {
  ...
  useEffect(() => {
    ...
    const postaviKorisnikaIZaduzenja = (k: any) => {
      setKorisnik(k);
      if (k.uloga === "student") {
        fetch(`https://overflowing-spirit-production-fde5.up.railway.app/api/zaduzenja/student/${k.id}`)
          .then((res) => res.json())
          .then(setZaduzenja)
    ...
    fetch("https://overflowing-spirit-production-fde5.up.railway.app/api/me", {
