...
export default function DodajPublikaciju() {
  ...
  const povuciPreview = async () => {
    ...
    setUcitava(true);
    try {
      const res = await fetch(`${BASE_URL}/api/proveri-knjigu`, {
        method: "POST",
  ...
  const spasiUPublikacije = async (e: React.FormEvent) => {
    ...
    const res = await fetch(`${BASE_URL}/api/publikacije`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        naziv,
          ...
          {ucitava ? (
            ...
            <div className="text-center">
              <img
                src={preview.slika}
                alt="Cover"
                className="h-64 shadow-lg mb-4 rounded mx-auto"
              />
              <p className="text-xs text-gray-600 line-clamp-3 px-4">
                {preview.opis}
              </p>
            </div>
          ) : (
