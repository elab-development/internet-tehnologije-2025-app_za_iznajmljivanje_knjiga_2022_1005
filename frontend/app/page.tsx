...
const CitatDana = () => {
  ...
  useEffect(() => {
    const ucitaj = async () => {
      try {
        ...
        const res = await fetch(`${BASE_URL}/api/citati`);
  ...
  return (
    <div className="relative w-full max-w-2xl mx-auto bg-white/70 backdrop-blur-md border-2 border-svetlo-plava p-8 rounded-[2.5rem] shadow-xl my-12 group overflow-hidden">
      <button
        onClick={prethodni}
        className="absolute left-4 top-1/2 -translate-y-1/2 z-10 p-2 rounded-full bg-white shadow-lg text-mint hidden sm:block"
      >
        <svg
      ...
      <button
        onClick={sledeci}
        className="absolute right-4 top-1/2 -translate-y-1/2 z-10 p-2 rounded-full bg-white shadow-lg text-mint hidden sm:block"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="24"
      ...
      >
        {citati.map((c, i) => (
          <div
            key={i}
            className="w-full flex-shrink-0 px-8 md:px-12 text-center"
          >
            <p className="text-lg md:text-xl italic font-medium text-slate-800 leading-relaxed mb-3">
              "{c.tekst}"
            </p>
            <span className="text-roze font-semibold tracking-widest text-xs md:text-sm uppercase">
              — {c.autor}
            </span>
          </div>
        ))}
...
export default function KatalogPage() {
  ...
  const osveziPodatke = () => {
    fetch(`${BASE_URL}/api/publikacije`)
      .then((res) => res.json())
      .then((data) => {
  ...
  const obrisiPublikaciju = async (id: number) => {
    ...
    const res = await fetch(`${BASE_URL}/api/publikacije/${id}`, {
      method: "DELETE",
    });
  ...
  const izvrsiZaduzivanje = () => {
    ...
    fetch(`${BASE_URL}/api/zaduzi-knjigu`, {
      method: "POST",
  ...
  return (
    <div className="py-6 px-4 max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-black text-tamno-plava uppercase">
          Katalog
        </h1>
        {isAdmin && (
          <Dugme
      ...
      {filtrirane.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtrirane.map((p) => (
            <div
              key={p.id}
              className="p-6 bg-white border-2 border-svetlo-plava rounded-2xl shadow-sm hover:shadow-md transition-shadow flex flex-col h-full"
            >
      ...
      {modalOtvoren && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div
            className={`bg-white w-full max-w-md p-8 rounded-[2rem] border-2 border-svetlo-plava shadow-2xl space-y-6 ${mrdni ? "animate-bounce" : ""}`}
          >
            <h2 className="text-xl font-black text-tamno-plava uppercase text-center">
              Zaduživanje
            </h2>
            <form
              className="space-y-4"
            ...
            >
              ...
              {feedback && (
                <p
                  className={`text-sm font-bold text-center p-2 rounded-lg ${feedback.uspeh ? "text-green-600 bg-green-50" : "text-red-600 bg-red-50"}`}
                >
                  {feedback.poruka}
                </p>
              )}
              <div className="flex flex-col gap-3">
                {!feedback?.uspeh && (
                  <Dugme naslov="Potvrdi" boja="zelena" tip="submit" />
                )}
                <Dugme
                  naslov="Zatvori"
                  boja="plava"
